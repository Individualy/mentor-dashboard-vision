...
const VerificationEmailByCode = () => {
  ...
  useEffect(() => {
    ...
    const tokenParam = searchParams.get('session');
  ...
  const handleComplete = async (value: string) => {
    ...
    // Get session token from URL or session storage
    const session = getSession(SESSION_KEYS.REGISTRATION);
    const searchParams = new URLSearchParams(location.search);
    ...
    console.log('Session information:', {
      sessionFromStorage: session ? 'exists' : 'missing',
      sessionTokenFromURL: sessionTokenParam || 'missing',
      sessionTokenFromSession: session?.token || 'missing'
    });
    ...
    try {
      ...
      if (response.error) {
        ...
        if (response.error.includes('Invalid') || response.error.includes('expired') || response.error.includes('required')) {
          // Try one more time with explicit session token
          try {
            console.log('Retrying verification with explicit session token...');
            // Create a direct axios call with all parameters explicitly set
            const API_URL = import.meta.env.VITE_API_URL || 'http://localhost:5001';
            const retryResponse = await axios.post(`${API_URL}/verify-email`, {
              email,
              code: value,
              session_token: sessionTokenParam || (session?.token || '')
            });

            console.log('Retry response:', retryResponse.data);

            if ((retryResponse.data as { message?: string }).message) {
              // Success on retry
